...
console.log('Fixing database with duplicate handling...');
...
db.serialize(() => {
  ...
  // Copy data from old table to new table, keeping only the first occurrence of each tenant
  db.run(`
    ...
    WHERE id IN (
      SELECT MIN(id) FROM remote_api_databases GROUP BY tenant_name
    )
  `, function(err) {
    if (err) {
    ...
    console.log('✅ Copied data to new table (deduplicated)');
